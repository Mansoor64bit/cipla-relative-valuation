...
![Relative Valuation Summary](screenshots/01_relative_valuation_summary.png)
...
1. Open `Cipla_Relative_Valuation_Model.xlsx` in Excel or Google Sheets.
2. Update figures on the `Raw Data - screener` tab as fresher market data comes in (price changes daily; financials change quarterly).
3. The `Relative Valuation` tab recalculates automatically.
4. Add or remove peers by adjusting which raw-data rows feed the peer table (row references in columns B–K).
